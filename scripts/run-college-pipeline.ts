...
const seedUcla = args.includes("--seed-ucla");
...
async function seedUclaSources() {
  const payload = {
    team_site_url: "https://uclabruins.com",
    roster_url: "https://uclabruins.com/sports/baseball/roster/2026",
    schedule_url: "https://uclabruins.com/sports/baseball/schedule/text",
  };
  ...
  await pool.query(
    `
      ...
      WHERE teamid = $1
    `,
    ["20054", payload.team_site_url, payload.roster_url, payload.schedule_url]
  );
...
async function processTeam(team: TeamRow): Promise<TeamSummary> {
  ...
  if (discover) {
    const discovery = await runScript("scripts/discover-college-team-sources.ts", [
      "--teamid",
      team.teamid,
    ]);
  ...
  const rosterRowsParsed = parseMetric(
    ingest.stdout,
    /Parsed\s+(\d+)\s+roster candidates/i
  );
...
async function main() {
  ...
  console.log(`UCLA seed step: ${seedUcla ? "ON" : "OFF"}`);

  if (seedUcla) {
    await seedUclaSources();
  }
