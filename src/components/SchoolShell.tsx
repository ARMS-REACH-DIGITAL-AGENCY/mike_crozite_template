// src/components/SchoolShell.tsx
// YAT?STATS — School microsite page shell
// Renders the full page structure (CSS, header with real-route nav, drawers, footer, JS).
// Each section route renders its content via the `children` prop.

import AccountDrawer from "@/components/AccountDrawer";
import SafeImage from "@/components/SafeImage";
import { getFirebaseConfigJSON } from "@/lib/firebase-config";

export interface SchoolShellProps {
  schoolName: string;
  location: string;
  crestUrl: string;
  resolvedHsid: string;
  subdomain: string;
  /** Route prefix: "" on custom domains, "/{hsid}" on preview/path-based access */
  navBase: string;
  /** Current section label shown below the school name (e.g. "WHERE THEY YAT?") */
  sectionLabel: string;
  /** Grad-class years for the filter drawer; computed by roster pages, empty for placeholders */
  gradClasses?: string[];
  children: React.ReactNode;
}

const NAV_ITEMS = [
  { thin: "WHERE THEY", bold: "YAT?", path: "/" },
  { thin: "ACTIVE ALUMNI", bold: "NEWS", path: "/active-alumni-news" },
  { thin: "NEXT-LEVEL", bold: "ALL-TIME LIST", path: "/next-level-all-time-list" },
  { thin: "THE", bold: "CURRENT TEAM", path: "/current-team" },
  { thin: "MENTORSHIP", bold: "MARKETPLACE", path: "/mentorship-marketplace" },
  { thin: "PCD ACTION", bold: "PARTNER PROGRAM", path: "/pcd-action-partner-program" },
  { thin: "", bold: "FAQ'S", path: "/faqs" },
];

function navHref(navBase: string, path: string): string {
  if (path === "/") return navBase || "/";
  return `${navBase}${path}`;
}
      ...
      <header className="yat-header" id="site-header">
        <div className="yat-container yat-topbar">
          ...
          <nav className="yat-topnav" aria-label="Top Navigation">
            {NAV_ITEMS.map((item) => (
              <a key={item.path} href={navHref(navBase, item.path)} className="yat-nav-pair">
        ...
        <div className="yat-schoolrow">
          ...
          <div className="yat-schooltext">
            ...
            <div className="big2" id="yatSectionLabel">{sectionLabel}</div>
          </div>
        </div>
        <div className="yat-hr" />
      ...
      <aside className="yat-drawer" id="drawerLeft">
        ...
        <div className="yat-drawer-content">
          ...
          <div className="yat-drawer-nav">
            {NAV_ITEMS.map((item) => (
              <a key={item.path} href={navHref(navBase, item.path)} className="yat-drawer-nav-item">
...
(function(){
  ...
  /* Hash-based backward compat — redirect old anchor links to real routes */
  (function(){
    var hashMap={'#sec-active':'','#sec-news':'/active-alumni-news','#sec-alltime':'/next-level-all-time-list','#sec-team':'/current-team','#sec-mentor':'/mentorship-marketplace','#sec-partner':'/pcd-action-partner-program','#sec-faq':'/faqs'};
    var h=window.location.hash;
    if(h&&Object.prototype.hasOwnProperty.call(hashMap,h)){
      var nb='${navBase}';
      var dest=nb+(hashMap[h]||'');
