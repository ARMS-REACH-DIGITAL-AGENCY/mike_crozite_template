...
export default function ZoomableCareerTimeline({ playerId, variant = 'combined' }: { playerId: string; variant?: 'combined' | 'images' | 'line' }) {
  ...
  const model = useMemo(() => {
    ...
    const prompts: Moment[] = [
      {
        id: 'prompt-memory',
        year: clamp(hsYear + 2, hsYear, end),
        label: 'Add',
        title: 'Add a dated memory',
        caption: `Place a real fan photo or baseball memory from ${firstName}'s journey on this line.`,
        kind: 'prompt',
      },
    ];
    ...
    const archive: Moment[] = [
      {
        ...
        year: hsYear,
        label: 'HS',
        title: 'High school card',
        caption: 'Return to the original flip card view.',
    ...
    return {
      ...
      span,
      ticks: buildTicks(hsYear, end, zoom),
      moments: [...archive, ...seasons, ...uploaded, ...prompts].sort((a, b) => a.year - b.year || Number(Boolean(a.cardMode)) - Number(Boolean(b.cardMode))),
    };
  ...
  return (
    <section className={`zt-shell ${variant === 'line' ? 'zt-shell-line' : ''}`} id="playerCareerStrip">
      ...
      <div className="zt-controls">
        <span>Zoom</span>
        <input aria-label="Zoom timeline" type="range" min="0.9" max="3.2" step="0.1" value={zoom} onChange={(event) => setZoom(Number(event.target.value))} />
      </div>
