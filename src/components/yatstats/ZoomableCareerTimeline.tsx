// src/components/yatstats/ZoomableCareerTimeline.tsx
// Golden Line v2: an interactive zoomable calendar timeline.
// The line is based on years from estimated DOB through today; moments land by date/year.
...
type Moment = {
  ...
  caption: string;
  src?: string;
  kind: 'cta' | 'prompt' | 'season' | 'archive' | 'upload';
};
...
function estimateDobYear(rows: StatRow[]) {
  ...
  if (!guesses.length) return new Date().getFullYear() - 24;
  return guesses[Math.floor(guesses.length / 2)];
}

function ticks(start: number, end: number, zoom: number) {
...
function cardImage(playerId: string, kind: 'then' | 'now') {
  return `${S3_BASE}/players/${kind}/${playerId}.jpg`;
}

function imgTag(src: string | undefined, title: string, kind: Moment['kind']) {
  if (!src) {
    const icon = kind === 'season' ? 'ri-baseball-line' : kind === 'prompt' ? 'ri-image-add-line' : 'ri-user-add-line';
    return `<span class="zt-empty"><i class="${icon}"></i><b>${kind === 'season' ? 'Season' : 'Memory'}</b></span>`;
  }
  return `<img src="${src}" alt="${title.replaceAll('"', '&quot;')}" loading="lazy" />`;
}
...
export default function ZoomableCareerTimeline({ playerId }: { playerId: string }) {
  ...
  useEffect(() => {
    ...
    fetch(`/api/player-season-stats?playerId=${encodeURIComponent(playerId)}`, { cache: 'no-store' })
      ...
      .then((data) => {
        ...
        const fallback = [...(data?.pitching || []), ...(data?.batting || [])];
  ...
  useEffect(() => {
    ...
    fetch(`/api/player-moments?playerId=${encodeURIComponent(playerId)}`, { cache: 'no-store' })
      .then((r) => (r.ok ? r.json() : null))
      .then((data) => { if (!off && data?.moments) setUploads(data.moments); })
      .catch(() => {});
    return () => { off = true; };
  }, [playerId]);
  ...
  const model = useMemo(() => {
    ...
    const statYears = stats.map((r) => yearOf(r.year)).filter((y): y is number => Boolean(y));
    ...
    const seasonMoments: Moment[] = stats
      .map((row) => {
        const year = yearOf(row.year);
        if (!year || !row.team) return null;
        const key = `${year}|${row.team}|${row.level}`;
    ...
    const uploadMoments: Moment[] = uploads.map((u) => {
      const year = u.photo_taken_year || yearOf(u.photo_taken_date) || dob + 15;
      return {
        id: `upload-${u.id}`,
        year,
        label: String(year),
        title: u.title || 'Fan memory',
        caption: u.caption || 'Fan-submitted Golden Line memory.',
        src: u.image_data_url,
        kind: 'upload',
      };
    });

    const prompts: Moment[] = [8, 14, 18].map((offset, i) => ({
      id: `prompt-${i}`,
      year: clamp(dob + offset, start, end),
      label: String(clamp(dob + offset, start, end)),
      title: ['First baseball memory', 'School-days memory', 'Next-level memory'][i],
      caption: `Help document a real dated moment from ${firstName}'s baseball story.`,
      kind: 'prompt',
    }));

    const archive: Moment[] = [
      { id: 'archive-then', year: statYears[0] || dob + 16, label: String(statYears[0] || dob + 16), title: 'The hometown chapter', caption: 'Archive image / early player chapter.', src: cardImage(playerId, 'then'), kind: 'archive' },
      { id: 'archive-now', year: statYears[statYears.length - 1] || today, label: String(statYears[statYears.length - 1] || today), title: 'Latest chapter', caption: 'Current or most recent player chapter.', src: cardImage(playerId, 'now'), kind: 'archive' },
      { id: 'cta', year: start, label: 'Add', title: 'Add a memory', caption: 'Place a fan memory on the calendar line.', kind: 'cta' },
    ];

    return { start, end, span, ticks: ticks(start, end, zoom), moments: [...archive, ...seasonMoments, ...uploadMoments, ...prompts].sort((a, b) => a.year - b.year) };
  ...
  return (
    <section className="zt-shell" id="playerCareerStrip">
      ...
      <div className="zt-controls">
        <span>Zoom</span>
        <input aria-label="Zoom timeline" type="range" min="0.8" max="3.8" step="0.1" value={zoom} onChange={(e) => setZoom(Number(e.target.value))} />
      </div>

      <div className="zt-window" ref={wrapRef}>
          ...
          {model.moments.map((m, i) => {
            ...
            return (
              <button
                type="button"
                key={m.id}
                className={`zt-moment zt-${m.kind} ${top ? 'zt-top' : 'zt-bottom'}`}
                style={{ left: left(m.year) }}
                onClick={() => m.kind === 'prompt' || m.kind === 'cta' ? openUpload() : undefined}
                dangerouslySetInnerHTML={{ __html: `<i class="zt-pin"></i><span class="zt-card"><span class="zt-photo">${imgTag(m.src, m.title, m.kind)}</span><span class="zt-copy"><b>${m.label}</b><strong>${m.title}</strong><em>${m.caption}</em></span></span>` }}
              />
            );
          })}
        </div>
      ...
      <style jsx>{`
        ...
        .zt-meta strong { font: 900 14px/1 Bebas Neue, Oswald, sans-serif; letter-spacing:.08em; }
        ...
        .zt-top .zt-card { bottom:14px; } .zt-bottom .zt-card { top:14px; }
        ...
        .zt-top .zt-pin:before { bottom:8px; } .zt-bottom .zt-pin:before { top:8px; }
        ...
        .zt-copy strong { margin-top:3px; color:#fff; font:900 11px/1 Bebas Neue, Oswald,sans-serif; letter-spacing:.05em; white-space:nowrap; overflow:hidden; text-overflow:ellipsis; }
