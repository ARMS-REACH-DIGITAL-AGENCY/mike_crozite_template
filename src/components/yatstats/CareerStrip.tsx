...
// The old hard-coded Journey Intro card has been removed. Row 3 now starts
// with real archive/upload moments only.
"use client";
...
function archiveMoments(playerId: string): CareerMoment[] {
  return [
    {
      id: "minors",
      year: "Minors",
...
export default function CareerStrip({ playerId }: { playerId: string }) {
  ...
  const moments = useMemo(() => {
    ...
    const realMoments = [...archiveMoments(playerId), ...uploads].sort((a, b) => {
  ...
  }, [playerId, playerName, submittedMoments]);
  ...
  return (
    <div className="gallery-strip golden-line-strip" id="playerCareerStrip">
      ...
      <style jsx>{`
        ...
        .gl-track { height: 100%; min-width: 740px; display: flex; align-items: stretch; gap: 10px; padding-bottom: 0; }
        ...
        @media (max-width: 760px) { .gl-track { min-width: 620px; gap: 8px; } .gl-slot { flex-basis: 108px; } .gl-card { width: 108px; } .gl-photo { width: 48px; height: 92px; } .gl-modal { grid-template-columns: 1fr; overflow-y: auto; } .gl-modal-media { min-height: 230px; max-height: 38vh; } .gl-modal-body { padding: 24px 20px 22px; } }
      `}</style>
    </div>
  );
}
