...
export default async function PlayerProfilePage({
  ...
  return (
    <>
      <style>{`
        /* Player profile CSS overrides and additions — shared vars/reset are in YatStyles via layout */
        :root{--bg:#0d0d0d;--fg:#f5f5f5;--muted:#999;--header-bg:rgba(13,13,13,.97);--crestH:60px;--card-bg:#1a1a1a}
        body.light-theme{--header-bg:rgba(244,244,244,.97)}
        .yat-container{max-width:1100px;margin:0 auto;padding:0 16px}
        /* HEADER overrides for player profile */
        .yat-header{backdrop-filter:blur(8px)}
        /* Sticky identity band: two-part layout */
        .yat-schoolrow{justify-content:space-between;gap:10px;max-width:1100px;margin:0 auto}
        .yat-schoolrow-id{display:flex;align-items:flex-start;gap:10px;flex-shrink:0}
        .yat-crest{transition:border-radius .2s,object-fit .2s}
        ...
        .career-strip-inner{width:100%;height:100%;padding:0;display:flex;gap:0;align-items:stretch;overflow-x:auto;-webkit-overflow-scrolling:touch;scroll-snap-type:x mandatory;scroll-behavior:smooth;scrollbar-width:none}
        ...
        /* FOOTER SPONSOR CTA (player-specific addition) */
        ...
        /* Hide GlobalSearchModal hero-right buttons — player page uses #btnSearch in topbar */
      ...
      {/* GLOBAL SEARCH MODAL — provides the #gsModal overlay. The .yat-hero-right action buttons
          inside GlobalSearchModal are hidden via CSS; we use #btnSearch in the topbar instead. */}
      <GlobalSearchModal />
